...
export default function DoctorDailyView({
    ...
    const miniDays = Array.from({ length: 7 }, (_, index) => {
        const date = new Date(weekStart);
        date.setDate(weekStart.getDate() + index);
        return date;
    });
    const shiftMiniWeek = (delta) => {
                            ...
                            >
                                ...
                                <div className="flex-1 px-6 py-4 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        {!appt.cancelled && (
                                            <>
                                                {appt.allowedActions?.cancel?.enabled && (
                                                ...
                                                <motion.button
                                                    ...
                                                    disabled={!appt.allowedActions?.openEncounter?.enabled}
                                                    ...
                                                    title={appt.allowedActions?.openEncounter?.reason || undefined}
                                                >
                                                    <span className="material-symbols-outlined text-sm">play_arrow</span>
                                                    {appt.allowedActions?.openEncounter?.label || (appt.rawStatus === 'in_consultation' ? 'Continue Encounter' : 'Open Encounter')}
                                                </motion.button>
                                            </>
                                        )}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex gap-2">
                            <button type="button" onClick={() => shiftMiniWeek(-1)} className="p-1 text-slate-400 hover:text-slate-600"><span className="material-symbols-outlined text-sm">chevron_left</span></button>
                            <button type="button" onClick={() => shiftMiniWeek(1)} className="p-1 text-slate-400 hover:text-slate-600"><span className="material-symbols-outlined text-sm">chevron_right</span></button>
                        </div>
                    </div>
                    <div className="grid grid-cols-7 gap-1">
                        ...
                        {miniDays.map((date) => (
                            <button
                                ...
                                className={`text-xs font-bold p-2 rounded-lg ${date.toDateString() === selectedDate.toDateString() ? 'bg-primary text-white' : 'text-slate-600 hover:bg-slate-100'}`}
                            >
                                {date.getDate()}
                            </button>
                        ))}
