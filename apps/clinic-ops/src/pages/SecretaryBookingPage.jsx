...
export default function SecretaryBookingPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await catalogService.getAll('visit_types');
      setVisitTypes(data || []);
    })();
  }, []);
  ...
  const handleBook = async () => {
    ...
    const { data, error } = await appointmentService.bookFromSlot({
      ...
      visitTypeId: selectedVisitType || null,
      status: 'scheduled',
    });
    setBooking(false);
    if (error) {
                    ...
                    {visitTypes.length > 0 && (
                      <div className="mb-3">
                        ...
                        <select
                          ...
                          className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                        >
                          <option value="">General Visit</option>
                          {visitTypes.map(vt => (
                            <option key={vt.id} value={vt.id}>{vt.name}{vt.default_duration_minutes ? ` (${vt.default_duration_minutes} min)` : ''}</option>
                          ))}
                        </select>
                      </div>
                    )}
