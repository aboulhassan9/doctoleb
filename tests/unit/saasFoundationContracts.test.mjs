...
describe('SaaS foundation contracts', () => {
  ...
  it('control-plane browser app uses control-plane env names and never references service-role keys', () => {
    ...
    assert.match(api, /auth\.refreshSession\(\)/);
    ...
    assert.match(consoleScreen, /AUTH_ERROR_CODES/);
    assert.match(consoleScreen, /Admin session expired/);
  ...
  it('tenant DB setup automation stores only secret references and records migration runs', () => {
    ...
    const analyticalReportRuntime = read('supabase/migrations/20260515210000_analytical_reports_runtime.sql');
    const analyticalReportRuntimeFix = read('supabase/migrations/20260517070000_analytical_reports_runtime_volatility_and_array_fix.sql');
    ...
    assert.match(secretRetryMigration, /create or replace function public\.admin_store_tenant_secret_ref/);
    assert.match(secretRetryMigration, /v_has_existing/);
    assert.match(secretRetryMigration, /from vault\.secrets/);
    assert.match(secretRetryMigration, /vault\.update_secret/);
    ...
    for (const source of [analyticalReportRuntime, analyticalReportRuntimeFix, tenantMigrationBundle]) {
      ...
      assert.doesNotMatch(source, /from unnest\(v_array_values\) as unnest/);
    }
    ...
    assert.match(storeTenantSecret, /summaryForStoreError/);
