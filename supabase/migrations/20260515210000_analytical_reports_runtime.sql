...
begin
  ...
  for i in 0 .. jsonb_array_length(v_group_by) - 1 loop
    ...
    v_resolved_keys := v_resolved_keys || v_alias;
    ...
    if v_granularity is not null then
      ...
      v_select_parts := v_select_parts || format('%I as %I', v_col, v_alias);
  ...
  for i in 0 .. jsonb_array_length(v_aggregations) - 1 loop
    ...
    if v_fn = 'count' then
      ...
      if v_fn = 'count_distinct' then
        v_select_parts := v_select_parts || format('count(distinct %I) as %I', v_col, v_alias);
      else
  ...
  for i in 0 .. jsonb_array_length(v_filters) - 1 loop
    ...
    if v_op = 'is_null' then
      v_filter_parts := v_filter_parts || format('%I is null', v_col);
    elsif v_op = 'not_null' then
      v_filter_parts := v_filter_parts || format('%I is not null', v_col);
      ...
      v_filter_parts := v_filter_parts || format(
  ...
  if 'is_archived' = any(v_allowed_columns) then
    v_filter_parts := v_filter_parts || 'is_archived = false';
  end if;
  ...
  for i in 0 .. jsonb_array_length(v_order_by) - 1 loop
    ...
    v_order_parts := v_order_parts || format('%I %s', v_ref, upper(v_dir));
  end loop;
