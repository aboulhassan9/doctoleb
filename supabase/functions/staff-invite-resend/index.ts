...
type StaffInviteResendInput = {
  staff_member_id?: unknown;
  client_request_id?: unknown;
};

type PreparedResendEvent = {
  event_id: string;
...
function parseInput(body: unknown) {
  ...
  const input = body as StaffInviteResendInput;
...
function mapDbError(message: string) {
  const known = [
    ...
    'STAFF_INVITE_NOT_RESENDABLE',
    'STAFF_INVITE_EMAIL_MISSING',
    'STAFF_INVITE_RESEND_EVENT_NOT_FOUND',
  ];

  return known.find((code) => message.includes(code)) || 'STAFF_INVITE_RESEND_FAILED';
}
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return invalid(503, 'STAFF_INVITE_RESEND_NOT_CONFIGURED', headers);
  }
  ...
  const { data: prepared, error: prepareError } = await serviceClient
    .rpc('create_staff_invite_resend_event', {
  ...
  if (!prepared.should_send) {
    if (prepared.status === 'sent') {
      ...
      if (staffError || !staffMember) {
        return invalid(409, 'STAFF_INVITE_RESEND_FAILED', headers);
      }
    ...
    return invalid(409, 'STAFF_INVITE_RESEND_IN_PROGRESS', headers);
  }
  ...
  if (inviteError) {
    await serviceClient.rpc('finish_staff_invite_resend_event', {
  ...
  const { data: staffMember, error: finishError } = await serviceClient
    .rpc('finish_staff_invite_resend_event', {
      p_event_id: prepared.event_id,
      p_status: 'sent',
      p_error_code: null,
    })
  ...
  if (finishError || !staffMember) {
    return invalid(409, 'STAFF_INVITE_RESEND_RECORD_FAILED', headers);
  }
