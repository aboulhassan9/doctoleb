...
function mapDbError(message: string) {
  const known = [
    ...
    'STAFF_MEMBER_NOT_REACTIVATABLE',
  ];

  return known.find((code) => message.includes(code)) || 'STAFF_MEMBER_REACTIVATE_FAILED';
}

function statusForError(error: string) {
  if (error === 'FORBIDDEN') return 403;
  if (error === 'INVALID_REQUEST') return 422;
  return 409;
}
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return invalid(503, 'STAFF_MEMBER_REACTIVATE_NOT_CONFIGURED', headers);
  }
  ...
  const { data: reactivated, error: reactivateError } = await serviceClient
    .rpc('reactivate_staff_member_domain_identity', {
  ...
  if (reactivateError || !reactivated) {
    const mappedError = mapDbError(reactivateError?.message || '');
    return invalid(statusForError(mappedError), mappedError, headers);
  }

  return json({ data: reactivated, error: null }, 200, headers);
});
