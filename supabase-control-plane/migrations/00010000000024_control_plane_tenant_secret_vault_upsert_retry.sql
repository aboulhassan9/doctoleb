-- DoctoLeb Control Plane · retry-safe tenant secret storage
...
select null, 'control_plane.tenant_secret_vault_upsert_retry_applied', jsonb_build_object(
...
where not exists (
  ...
  where event_type = 'control_plane.tenant_secret_vault_upsert_retry_applied'
);
